...
def train(model, train_gen, criterion, optimizer, device, epoch, writer, args):
    ...
    with torch.set_grad_enabled(True):
        for batch_idx, data in enumerate(loader):
            ...
            dec_traj = model(inputs=[input_traj, input_speed], targets = target_traj, mask = None, frames=frames)
...
def val(model, val_gen, criterion, device, epoch, writer, args):
    ...
    with torch.set_grad_enabled(False):
        for batch_idx, data in enumerate(loader):
            ...
            # else:
            #     print("Frames not in val")    
            dec_traj  = model(inputs=[input_traj, input_speed], targets = target_traj, mask = None, training=False, frames=frames)
            dec_loss = criterion(dec_traj, target_traj)            
...
def test(model, test_gen, criterion, device, epoch, writer, args):
    ...
    with torch.set_grad_enabled(False):
        for batch_idx, data in enumerate(loader):     
            ...
            target_traj = data['target_y'].to(device)
            frames = data['frames'] if 'frames' in data else None
            dec_traj  = model(inputs=[input_traj, input_speed], targets = target_traj, mask = None, training=False, loop=1, frames=frames)
            dec_loss = criterion(dec_traj, target_traj)            
